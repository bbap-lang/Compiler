...
public class Concatenate : IFunction {
    public Result<int> Matches(IType[] inputs, IType[] outputs, int line) {
        if (outputs.Length > 1) return Error(line, "The function 'CONCATENATE' has only one return value.");
        ...
        if (output is not null && !TypeCollection.StringType.IsCastableTo(output))
            return Error(line,
                         "In the function call of 'CONCATENATE', the variable for the return value is not a string.");


        foreach ((IType type, int index) in inputs.Select((t, i) => (t, i))) {
            if (!type.IsCastableTo(TypeCollection.StringType))
                return Error(line,
                             $"The parameter type {type.Name}  is not castable to {TypeCollection.StringType} in the function call of 'CONCATENATE'.");
        }
