...
public static class IfParser {
    public static Result<IExpression> Run(ParserState state, int line) {
        ...
        Result<IToken> elseResult = state.Next(typeof(ElseToken));
        if(!elseResult.TryGetValue(out IToken? elseToken)) {
            state.Revert();
    ...
    public static Result<IExpression> ParseElse(ParserState state, int line) {
        ...
        if(!startTokenResult.TryGetValue(out IToken? startToken)) {
            return startTokenResult.ToErrorResult();
        }

        if (startToken is IfToken) {
            return Run(state, startToken.Line);
        }
