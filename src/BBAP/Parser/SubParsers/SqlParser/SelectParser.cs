...
public static class SelectParser {
    public static Result<IExpression> Run(ParserState state, out IToken lastToken, Type[] endTokenTypes, int line) {
        ...
        if (fieldTokenResult.TryGetValue(out var starToken)) {
            ...
            if (returnFields.Count == 0) {
                Error(line, "Expected at least one field to return");
            }
        }
        ...
        if (!lastTokenResult.TryGetValue(out lastToken)) {
            return lastTokenResult.ToErrorResult();
        }
