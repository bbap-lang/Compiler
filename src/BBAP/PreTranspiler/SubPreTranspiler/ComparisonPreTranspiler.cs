...
public static class ComparisonPreTranspiler {
    public static Result<IExpression[]> Run(PreTranspilerState state, ComparisonExpression comparisonExpression) {
        ...
        if (lastLeftEx is not ISecondStageValue lastLeft) throw new UnreachableException();

        if (lastRightEx is not ISecondStageValue lastRight) throw new UnreachableException();
        ...
        if (!leftType.SupportsOperator(comparisonExpression.ComparisonType.ToSupportedOperator()))
            return Error(line,
                         $"The type {leftType.Name} does not support the operator {comparisonExpression.ComparisonType}.");

        if (!rightType.SupportsOperator(comparisonExpression.ComparisonType.ToSupportedOperator()))
            return Error(line,
                         $"The type {rightType.Name} does not support the operator {comparisonExpression.ComparisonType}.");

        Result<IType> typeResult = state.Types.Get(line, "BOOL");
        if (!typeResult.TryGetValue(out IType? typeBool)) throw new UnreachableException();
        ...
        var newComparison
            = new SecondStageCalculationExpression(line, new TypeExpression(line, typeBool), calculationType, lastLeft,
                                                   lastRight);
