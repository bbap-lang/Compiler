...
public static class ConditionPreTranspiler {
    public static Result<( IExpression[] additional, ISecondStageValue Condition)> Run(IExpression baseCondition,
        ...
        if (conditionExpression is not ISecondStageValue condition) throw new UnreachableException();

        Result<IType> typeBoolResult = state.Types.Get(condition.Line, Keywords.Boolean);
        if (!typeBoolResult.TryGetValue(out IType? typeBool)) throw new UnreachableException();
        ...
        if (condition is SecondStageValueExpression conditionValue) {
            var trueExpression = new SecondStageValueExpression(conditionValue.Line,
                                                                new TypeExpression(conditionValue.Line, typeBool),
                                                                new BooleanValueExpression(conditionValue.Line, true));
            condition = new SecondStageCalculationExpression(conditionValue.Line,
                                                             new TypeExpression(conditionValue.Line, typeBool),
                                                             SecondStageCalculationType.Equals, conditionValue,
                                                             trueExpression);
        }
