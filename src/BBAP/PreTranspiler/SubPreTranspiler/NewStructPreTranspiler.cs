...
public class NewStructPreTranspiler {
    public static Result<IExpression[]> Run(StructSetExpression structSetExpression, PreTranspilerState state) {
        Result<IVariable> structVarResult = state.GetVariable(structSetExpression.Variable.Variable.Name, structSetExpression.Line);
        if(!structVarResult.TryGetValue(out IVariable? structVar)) {
            throw new UnreachableException();
        }

        var generalType = structVar.Type;
        ...
        var declareExpression = new DeclareExpression(structSetExpression.Line, structVarExpression, structTypeExpression, null);
        ...
        Result<IExpression[]> setExpressionsResults = structSetExpression.Fields.Select(x => {
            ...
            if (!valueResult.TryGetValue(out IExpression[]? valueExpressions)) {
            ...
            if(valueExpression is not ISecondStageValue value) {
                throw new UnreachableException();
            }
        ...
        IExpression[] expressions = ArrayBuilder<IExpression>.Concat(additionalExpressions).Concat(setExpressions).Build();
