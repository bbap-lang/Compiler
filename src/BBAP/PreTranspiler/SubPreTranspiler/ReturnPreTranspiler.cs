...
public static class ReturnPreTranspiler {
    public static Result<IExpression[]> Run(ReturnExpression returnExpression, PreTranspilerState state) {
        ...
        foreach (IExpression value  in returnExpression.ReturnValues) {
            ...
            if(newValueGeneral is not ISecondStageValue newValue) {
        ...
        foreach ((IVariable returnVariable, ISecondStageValue value) in returnVariables.Select((x, i) => (x, newValues[i]) )) {
            if (!value.Type.Type.IsCastableTo(returnVariable.Type)) {
                return Error(returnExpression.Line, $"The type of the return value ({value.Type.Type.Name}) does not match the type of the return variable ({returnVariable.Type.Name})");
            }
