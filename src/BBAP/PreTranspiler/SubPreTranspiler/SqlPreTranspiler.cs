...
public static class SqlPreTranspiler {
    public static Result<IExpression[]> RunSelect(SqlSelectExpression sqlSelectExpression, PreTranspilerState state) {
        VariableExpression[] rawTables
            = sqlSelectExpression.Joins.Select(x => x.Table).Append(sqlSelectExpression.From).ToArray();

        Result<IVariable[]> tablesResult = rawTables.Select(x => GetVariableExpressionFromTable(x, state)).Wrap();

        if (!tablesResult.TryGetValue(out IVariable[]? tables)) return tablesResult.ToErrorResult();
        ...
        foreach (VariableExpression variableExpression in sqlSelectExpression.OutputFields) {
            Result<ISecondStageSqlValueExpression> variableResult
                = RunVariableExpression(state, variableExpression, tables);
            if (!variableResult.TryGetValue(out ISecondStageSqlValueExpression? variableValue))
                return variableResult.ToErrorResult();

            if (variableValue is not SecondStageSqlVariableExpression variable) throw new UnreachableException();

            outputFields.Add(variable);
        }

        Result<IVariable> fromResult
            = state.GetVariable(sqlSelectExpression.From.Variable, sqlSelectExpression.From.Line);
        if (!fromResult.TryGetValue(out IVariable? from)) return fromResult.ToErrorResult();
        ...
        Result<SecondStageJoinExpression[]> joinsResult = sqlSelectExpression.Joins.Select(join => {
            ...
            VariableExpression tableExpression = join.Table with { Variable = table };
        ...
        if (!joinsResult.TryGetValue(out SecondStageJoinExpression[]? joins)) return joinsResult.ToErrorResult();
        ...
        var newSelectExpression = new SecondStageSelectExpression(sqlSelectExpression.Line,
    ...
    private static Result<IVariable> GetVariableExpressionFromTable(VariableExpression variableExpression,
        ...
        if (variable.Type is not TableType
         && !(variable.Type is AliasType aliasType && aliasType.GetRealType() is TableType))
            return Error(variableExpression.Line, $"The variable '{variable.Name}' is not a table type.");

        return Ok(variable);
    }

    private static Result<ISecondStageSqlValueExpression> RunFilter(IExpression value,
        PreTranspilerState state,
        IVariable[] tables) {
        return value switch {
            MathCalculationExpression mathCalculationExpression =>
                RunMathCalculationExpression(state, mathCalculationExpression, tables),
    ...
    private static Result<ISecondStageSqlValueExpression>
        WrapInValue(PreTranspilerState state, IExpression expression) {
    ...
    private static Result<ISecondStageSqlValueExpression> RunVariableExpression(PreTranspilerState state,
        ...
        Result<IVariable> topVariableResult = state.GetVariable(rawTopVariable, variableExpression.Line);
        ...
        if (GetTableType(topVariable) is TableType tableType) {
            ...
            if (variableChain.Length == 1)
                return Error(variableExpression.Line,
                             $"The table variable '{topVariable.Name}' must be followed by a field.");

            IType currentType = tableType.ContentType;
            if (currentType is AliasType aliasTypeTemp) currentType = aliasTypeTemp.GetRealType();

            variable = topVariable;
            foreach (IVariable localVariable in variableChain.Skip(1)) {
                if (currentType is not StructType structType)
                    return Error(variableExpression.Line,
                                 $"The variable/field '{currentType.Name}' is not a struct type and can therefor not contain the field '{localVariable.Name}'.");

                Variable? currentField = structType.Fields.FirstOrDefault(x => x.Name == localVariable.Name);
                if (currentField is null)
                    return Error(variableExpression.Line,
                                 $"The struct type '{structType.Name}' does not contain the field '{localVariable.Name}'.");
    ...
    private static IType GetTableType(IVariable topVariable) {
        if (topVariable.Type is TableType tableType) return tableType;
    ...
    private static Result<ISecondStageSqlValueExpression> RunComparisonExpression(PreTranspilerState state,
        ComparisonExpression comparisonExpression,
        IVariable[] tables) {
        Result<ISecondStageSqlValueExpression> leftResult = RunFilter(comparisonExpression.Left, state, tables);
        if (!leftResult.TryGetValue(out ISecondStageSqlValueExpression? left)) return leftResult.ToErrorResult();

        Result<ISecondStageSqlValueExpression> rightResult = RunFilter(comparisonExpression.Right, state, tables);
        if (!rightResult.TryGetValue(out ISecondStageSqlValueExpression? right)) return rightResult.ToErrorResult();
        ...
        if (left.Type.Type.IsCastableTo(right.Type.Type))
            ...
            return Error(comparisonExpression.Line,
        ...
        if (!operatorResult.TryGetValue(out SqlFilterOperator @operator)) return operatorResult.ToErrorResult();

        return Ok<ISecondStageSqlValueExpression>(new SecondStageSqlFilterExpression(comparisonExpression.Line, type,
                                                   @operator, left, right));
    }

    private static Result<ISecondStageSqlValueExpression> RunBooleanExpression(PreTranspilerState state,
    ...
    private static Result<ISecondStageSqlValueExpression> RunMathCalculationExpression(PreTranspilerState state,
        ...
        return Ok<ISecondStageSqlValueExpression>(new SecondStageSqlFilterExpression(mathCalculationExpression.Line,
                                                   type,
                                                   @operator, left, right));
    }
    ...
    private static Result<SqlFilterOperator> GetOperator(BooleanType booleanExpressionBooleanType, int line) {
        return booleanExpressionBooleanType switch {
            ...
            BooleanType.Xor =>
                Error(line,
                      "Xor is not supported in sql."), // TODO: Support xor, with changing the structure of the sql filter expression
            _ => throw new UnreachableException()
        };
    }
